/**
 * Created by cailong on 2015/10/13.
 */
/*global define*/
'use strict';
define([
    ...
    (function(view){
        ...
        view.tableExport = function(tableId, filename, type){
            var doc = view.document,
                _table = doc.getElementById(tableId),
                charSet = doc.characterSet;

            //improved by cailong at 151013
            //exclude display:none rows
            var table = _table.cloneNode(true);
            for(var i = 0; i < table.rows.length; i++){
                var currRow = table.rows[i];
                if(currRow.style.display === 'none'){
                    currRow.parentNode.removeChild(currRow);
                    //rows.length - 1, so i should - 1 also
                    i--;
                }
            }
